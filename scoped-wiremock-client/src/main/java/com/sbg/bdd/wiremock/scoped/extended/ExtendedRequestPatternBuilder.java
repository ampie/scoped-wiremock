...
public class ExtendedRequestPatternBuilder<T extends ExtendedRequestPatternBuilder> extends RequestPatternBuilder {
    ...
    private UrlPattern calculateUrlPattern(EndPointRegistry endPointRegistry) {
        String path = this.urlInfo;
        if (isPropertyName(path)) {
            try {
                URL uri = endPointRegistry.endpointUrlFor(path);
                path = uri.getPath();
            } catch (Exception e) {
...
//    }
    ...
    public void prepareForBuild(EndPointRegistry endPointRegistry) {
        if (urlPattern == null) {
