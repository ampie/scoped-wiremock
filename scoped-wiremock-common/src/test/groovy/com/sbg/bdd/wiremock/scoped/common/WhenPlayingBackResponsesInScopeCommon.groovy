...
abstract class WhenPlayingBackResponsesInScopeCommon extends ScopedWireMockCommonTest {
    def 'Should play back the example_journal directory as specified by a recording mapping in the global scope as per the global journal mode'() {
        ...
        response0 == 'hello 0'
        response1 == 'hello 1'
        and: 'The correlation path pattern should be an exact match to ensure accurate playback'
        wireMock.allStubMappings().mappings[1].request.headers[HeaderName.ofTheCorrelationKey()].expected == globalScope.correlationPath + '/nested1_recording_scope/:John_Smith'
    }
    ...
    def 'Should play back the example_recordings directory as specified by a playback mapping in the global scope'() {
        ...
        response0 == 'hello 0'
        response1 == 'hello 1'
        and: 'The correlation path pattern should be pattern match to allow for the recording directory to be moved around'
        wireMock.allStubMappings().mappings[1].request.headers[HeaderName.ofTheCorrelationKey()].expected == globalScope.correlationPath + '/.*:John_Smith'
