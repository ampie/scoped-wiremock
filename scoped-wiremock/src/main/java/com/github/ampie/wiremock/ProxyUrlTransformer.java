...
public class ProxyUrlTransformer extends ResponseDefinitionTransformer {
    ...
    private static int getNumberOfSegments(Parameters parameters) {
        if (parameters.containsKey("segmentsToUse")) {
            return parameters.getInt("segmentsToUse");
        } else if (parameters.containsKey("numberOfSegments")) {
    ...
    private static boolean ignoreTrailingSegments(Parameters parameters) {
        return should(parameters, "ignoreTrailingSegments", "ignoreLeadingSegments");
    }

    private static Boolean should(Parameters parameters, String strategy, String antiStrategy) {
        if (parameters.containsKey(strategy)) {
            return parameters.getBoolean(strategy);
        }else if(parameters.containsKey(antiStrategy)){
            //TODO yeah reevaluate this, maybe split parameters into:
            // ignore/use and trailing/leading
            return !parameters.getBoolean(antiStrategy);
        }else{
            return false;
        }
    }
    ...
    private static Boolean ignoreLeadingSegments(Parameters parameters) {
        return should(parameters,  "ignoreLeadingSegments","ignoreTrailingSegments");
    }

    private static Boolean useLeadingSegments(Parameters parameters) {
        return should(parameters, "useLeadingSegments","useTrailingSegments");
    }

    private static Boolean useTrailingSegments(Parameters parameters) {
        return should(parameters, "useTrailingSegments", "useLeadingSegments");
    }
